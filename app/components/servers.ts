...
// Demo data for the interactive popover, matching the servers in the Paper boards.
...
export const SERVERS: Server[] = [
  {
    ...
    uptime: 'up 3h 12m',
    context: { agent: 'claude', text: 'providence · up 3h' },
  ...
  },
  {
    ...
    uptime: 'idle 5h',
    context: { agent: 'codex', text: 'lisbon · idle 5h' },
  ...
  },
  {
    ...
    cleanUp: { reason: 'deleted', note: 'Worktree deleted 2 days ago', suggested: true },
  },
]
...
// Memory history for the detail chart (60 samples, 76px tall). The steady trace's
// wobble is reused for every server, positioned by its current memory against a 2 GB line.
const WOBBLE = [
  40, 39.2, 38.2, 39.3, 39.6, 40.2, 40.1, 38.6, 38, 37.2, 38.1, 38.8, 38.8, 38, 37.7, 36.5, 35.7, 35.8, 37, 37.6,
  36.5, 36.5, 34.5, 34.8, 35.5, 36.3, 36.4, 35.4, 35, 33.6, 34.3, 37.3, 37.9, 36.5, 35, 33.7, 32.2, 32.1, 32.8, 33.2,
  33.7, 33.4, 31.8, 30.9, 31.1, 31.2, 32, 32, 31.6, 31, 29.3, 30, 30.1, 30.5, 31.5, 30.6, 30, 28.3, 28, 29.1,
]
...
export function memoryChart(server: Server) {
  if (server.chart === 'leaking') {
    return { line: LEAK, end: 4.8, limit: 25.5, top: '3 GB', topAmber: false }
  }
  const level = 76 - (server.memory / 2000) * 75
  const amplitude = server.chart === 'flat' ? 0.15 : 1
  const last = WOBBLE[WOBBLE.length - 1]
  const points = WOBBLE.map((y, i) => [((i * 328) / (WOBBLE.length - 1)).toFixed(1), (level + (y - last) * amplitude).toFixed(1)])
  return {
    line: `M${points.map(([x, y]) => `${x} ${y}`).join(' L')}`,
    end: Number(points[points.length - 1][1]),
    limit: 1,
    top: '2 GB',
    topAmber: true,
  }
...
// CPU history (bar heights out of 28px), scaled per server.
const CPU = [
  4.3, 2.1, 3.6, 3.2, 3.1, 2.6, 4.1, 4.5, 2.7, 3.4, 1.1, 3.6, 16, 17.3, 4.1, 2, 2.4, 3.5, 0.9, 2.6, 1.5, 1.3, 1.1,
  3.9, 1.3, 1.8, 2.4, 4.3, 1.2, 2.6, 3, 4.3, 4.1, 4.2, 1.9, 2.5, 2.2, 4.3, 4.6, 1.4, 1.5, 14.3, 14.4, 15.3, 3.1, 1.9,
  0.9, 2.5, 2.3, 3.1, 4.6, 3.5, 2.9, 3.3, 3.5, 1.1, 4.4, 3.9, 4.3, 4,
]

export function cpuBars(server: Server) {
  const scale = server.cpu === 0 ? 0.15 : Math.min(1.6, 0.5 + server.cpu / 24)
  return CPU.map((h) => Math.max(0.6, Math.min(27, h * scale)))
}
