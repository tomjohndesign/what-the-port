...
The script embeds the public configuration, builds the app, signs its bundled framework and helpers, optionally notarizes and staples it, packages it as a disk image (notarized the same way), verifies that the Keychain signing key matches the embedded public key, and creates:
...
- `.build/WhatThePort.dmg` — the website download: a signed, notarized disk image with the app beside an Applications shortcut ([`make-dmg.sh`](make-dmg.sh)).
...
cp WhatThePort/.build/WhatThePort.dmg public/WhatThePort.dmg
```

Both paths are gitignored. Production builds check that the download is a disk image and that the feed's newest archive is published and matches `Info.plist`.
...
```bash
vercel metrics vercel.request.count -p whattheport --prod -s 30d -g 1d \
  -f "request_path eq '/WhatThePort.dmg'" --group-by referrer_hostname
```
